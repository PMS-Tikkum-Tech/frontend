      ...
      <section className="relative overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-br from-[#1E2746] via-[#273965] to-[#2C62A5] p-4 text-white shadow-sm sm:rounded-3xl sm:p-6">
        ...
        <div className="relative flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <button
            ...
            className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-white px-4 text-sm font-semibold text-[#1E2746] hover:bg-slate-100 sm:w-auto"
          >
            <Plus size={16} />
            Tambah Pesan
          </button>
          ...
          <div className="flex min-w-0 items-center gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <p className="hidden text-xs text-slate-500 sm:block">
                Saring pesan berdasarkan status, properti, dan jenis komunikasi.
              </p>
            </div>
          </div>
          <button
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-12">
            ...
            <label className="min-w-0 xl:col-span-3">
              ...
              <select
                ...
                onChange={(event) =>
                  setSortBy(
                    event.target.value as "newest" | "oldest" | "scheduled",
                  )
                }
                className="h-11 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 text-sm text-slate-700 focus:border-blue-400 focus:bg-white focus:outline-none"
              >
            ...
            <div className="min-w-0 xl:col-span-3">
              ...
              <button
                type="button"
                role="switch"
                aria-checked={visitOnly}
                onClick={() => setVisitOnly((prev) => !prev)}
                className={`flex h-11 w-full items-center justify-between gap-3 rounded-lg border px-3 text-sm font-medium transition ${
                  visitOnly
              ...
              >
                <span className="truncate">
                  {visitOnly ? "Permintaan Kunjungan" : "Semua Komunikasi"}
                </span>
                <span
                  aria-hidden="true"
                  className={`relative h-5 w-9 shrink-0 rounded-full transition ${
                    visitOnly ? "bg-sky-500" : "bg-slate-300"
                  }`}
                >
                  <span
                    className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow-sm transition ${
                      visitOnly ? "left-[18px]" : "left-0.5"
                    }`}
                  />
                </span>
              </button>
