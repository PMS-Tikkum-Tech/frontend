...
export default function StatCard({ title, value, icon }: Props) {
  return (
    <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm transition hover:shadow-md sm:p-6">
      <div className="flex items-start justify-between gap-3 sm:items-center">
        <div>
          <p className="text-xs font-medium text-slate-500 sm:text-sm">{title}</p>
          <h3 className="mt-1 text-xl font-semibold text-slate-800 sm:text-2xl">
            {value}
          </h3>
        </div>
