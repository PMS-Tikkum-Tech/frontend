...
export default function PropertyCard({
  ...
  return (
    <article
      ...
      className={`group flex h-full cursor-pointer flex-col overflow-hidden rounded-[18px] border border-[#e2dfff] bg-white p-1 shadow-[0_8px_24px_rgba(52,35,184,0.07)] outline-none transition hover:-translate-y-1 hover:shadow-[0_16px_36px_rgba(52,35,184,0.13)] focus-visible:ring-2 focus-visible:ring-[#3423b8] sm:rounded-[20px] ${
        isSelected
    ...
    >
      <div className="relative">
        <ImageCarousel
          ...
          alt={property.name}
          className="aspect-[4/3] rounded-[14px] sm:rounded-[16px]"
        />
        <div className="absolute left-3 top-3 flex flex-wrap gap-2">
          {property.availableUnits > 0 ? (
            <span className="rounded-full bg-white/95 px-2.5 py-1 text-[10px] font-semibold text-slate-900 shadow-sm sm:px-3 sm:text-[11px]">
              Tersedia sekarang
            </span>
          ) : null}
      ...
      <div className="flex flex-1 flex-col px-3 pb-3 pt-3 sm:pt-4">
        <div className="flex items-start justify-between gap-2 sm:gap-3">
          <h2 className="line-clamp-2 text-sm font-semibold leading-snug text-[var(--color-text-primary)] sm:text-base">
            {property.name}
          </h2>
          <span className="shrink-0 rounded-full bg-slate-100 px-2 py-1 text-[10px] font-semibold text-slate-700 sm:text-[11px]">
            {property.propertyTypeLabel}
          </span>
        </div>
        <p className="mt-2 flex min-h-10 items-start gap-1.5 text-[11px] leading-relaxed text-[var(--color-text-secondary)] sm:text-xs">
          <MapPin size={14} className="mt-0.5 shrink-0 text-[#6f63d7]" />
          <span className="line-clamp-2">{property.address}</span>
        </p>

        <div className="mt-3 grid grid-cols-2 gap-2 text-[11px] sm:mt-4 sm:text-xs">
          <div className="rounded-xl bg-[#f5f3ff] px-2.5 py-2 sm:px-3 sm:py-2.5">
          ...
          <div className="rounded-xl bg-slate-50 px-2.5 py-2 sm:px-3 sm:py-2.5">
        ...
        <div className="mt-auto flex items-end justify-between gap-3 border-t border-slate-100 pt-3 sm:pt-4">
          <div>
            ...
            <p className="mt-1 text-sm font-bold text-slate-900 sm:text-sm">
              {property.priceLabel}
              <span className="ml-1 font-normal text-slate-400">/bulan</span>
            </p>
          </div>
          <span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#3423b8] text-white transition group-hover:bg-[#24147d] sm:h-9 sm:w-9">
            <ArrowRight size={15} />
          </span>
        </div>
      </div>
